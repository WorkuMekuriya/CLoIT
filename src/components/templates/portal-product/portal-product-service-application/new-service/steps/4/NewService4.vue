    ...
    <div>
      ...
      <section class="section-container">
        ...
        <div class="card-list-container">
          <Card :card-list="infoCard" @click="toggleSelect" class="card-spacing" />
        </div>
        <div>
          <div class="flex items-center text-center checkbox-container">
            <div class="section-checkbox-title">정보입력</div>
            <div v-if="selectedIndex == 0">
              <CheckboxButton :value="true" label="가입자와 정보가 동일합니다." class="checkbox-spacing" />
            </div>
          </div>

          <div v-if="selectedIndex == 0" class="grid-container">
            <div v-for="(form, index) in informationForm" :key="index" class="grid-item">
              <TextField class="textfield" :label="form.name" :placeholder="form.name"
                :bottomLeftDescription="form.description" bottomRightDescription="" />
            </div>
          </div>

          <div v-if="selectedIndex == 1" class="grid-container">
            <div v-for="(form, index) in organizationInfo" :key="index" class="grid-item">
              <TextField class="textfield" :label="form.name" :placeholder="form.placeholder || form.name"
                :bottomLeftDescription="form.description" bottomRightDescription="" />

            </div>
          </div>
          <div v-if="selectedIndex == 1">
            <div v-for="(form, index) in organizationInfo_2" :key="index" class="grid-item">
              <TextField class="textfield" :label="form.name" :placeholder="form.placeholder || form.name"
                :bottomLeftDescription="form.description" bottomRightDescription="" />
          ...
          <div v-if="selectedIndex == 1" class="grid-container">
            <div v-for="(form, index) in organizationInfo_3" :key="index" class="grid-item">
              <TextField class="textfield" :label="form.name" :placeholder="form.placeholder || form.name"
                :bottomLeftDescription="form.description" bottomRightDescription="" />
      ...
      <section class="section-container">
        ...
        <div class="grid-container">
          <div v-for="(account, index) in refundAccountInfo" :key="index" class="grid-item">
            <TextField class="textfield" :label="account.name" :placeholder="account.name" bottomLeftDescription=""
              bottomRightDescription="" />
          </div>
        </div>

        <TextField class="textfield" label="시스템 사용 개시일" placeholder="2024/04/21" bottomLeftDescription=""
          bottomRightDescription="" />

        <TextField class="textfield" label="원하는 Sub-Domain" placeholder="원하는 Sub-Domain" bottomLeftDescription=""
          bottomRightDescription="" />
      </section>
      ...
      <section class="section-container">
        <div class="section-title">환급계좌정보</div>
        <TextArea class="textfield" label="Message" bottomLeftDescription="자유롭게 하고싶은 말을 적어주세요."
          bottomRightDescription="" />
      </section>
    </div>
    <SelectedService :products="products" :options="options" options-total="{}" product-total="{}"
      label="Next : Checkout" total="{}" @emitButtonClick="moveToStep5" />
  </div>
</template>
...
const informationForm = ref([
  ...
  { name: 'Preferred Language', description: '기본적으로 영어, 한국어, 프랑스어, 독일어, 스페인어가 제공됩니다. 이외 원하는 언어가 있는 경우 적어주세요.' },
]);
...
const organizationInfo_2 = ref([
  { name: 'Location', placeholder: 'Street, Building Number (Optional)', description: '' },
  { name: '', placeholder: 'More Address Information (Optional)', description: '' },
