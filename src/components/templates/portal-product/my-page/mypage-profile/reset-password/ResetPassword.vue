<template>
  <div>
    <div style="display: flex;">
      ...
      <span style="color: black; text-decoration: none !important;">Reset Password</span>
    </div>
    <div class="header-title">Resetpassword</div>
    <div class="form-container">
      <div v-for="(field, index) in resetPasswordForm" :key="index" class="grid-item">
        <TextField class="textfield" :label="field.label" :placeholder="field.placeholder || field.label"
          :bottomLeftDescription="field.description" />
      </div>
      <div class="verification-code-label">Verification Code</div>
      <div class="verification-code-container">
        <q-input borderless placeholder="Verification Code" class="verification-code-input" />
        <q-icon name="cached" size="24px" class="verification-code-icon" />
        <div class="verification-code-text">4986</div>
      </div>
    </div>
    <div class="button-container">
      <RoundedButton type="Primary" :isLoading="false" label="Reset password" leftIcon="" :rightIcon="false"
        buttonWidth="186px" @click="resetPassword" />
    </div>
    <DialogComponent />
  </div>
</template>
