<template>
  <div class="q-mb-xl">
    ...
    <template
      v-for="(tableHeaderGroup, index) in tableHeaderGroups"
      :key="index"
    >
      <div>
        <p class="text-center display-1 q-py-md">
          {{ tableHeaderGroup.title }}
        </p>
        <div class="flex justify-around q-mx-xl">
          <template
            v-for="(
              tableHeader, tableHeaderIndex
            ) in tableHeaderGroup.tableHeaders"
            :key="tableHeaderIndex"
          >
            <component :is="tableHeader.component" v-bind="tableHeader.props" />
          </template>
        </div>
      </div>
    </template>
