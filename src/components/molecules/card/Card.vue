<template>
    <q-card v-for="(card, index) in cardList" class="card-container" @click="toggleCardType(index, $event)"
        :class="{ 'selected-state': isCardSelected(index), 'unselected-state': !isCardSelected(index), 'sm-card-container': card.size == 'sm', 'md-card-container': card.size == 'md' }"
        style="border-radius: 36px;">
        <div class="card-header" :class="{ 'md-card-header': card.size == 'md', 'sm-card-header': card.size == 'sm' }">
            ...
            <div v-if="card.size == 'md'" class="header-title">
                <div class="card-title" style="font-weight: normal;">{{ card.price }}</div>
            </div>
            <div class="check-box"
                :class="{ 'selected-check-box': isCardSelected(index), 'sm-selected-check-box': card.size == 'sm' & isCardSelected(index) }"
                @click="toggleCardType(index, $event)">
                <svg v-if="card.size == 'md' & !isCardSelected(index)" xmlns="http://www.w3.org/2000/svg" width="24"
                ...
                <svg v-if="card.size == 'lg' | (card.size == 'md' & isCardSelected(index)) || (card.size == 'sm' & !isCardSelected(index))"
                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M20 6L9 17L4 12" stroke="#101828" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
                <svg v-if="card.size == 'sm' & isCardSelected(index)" xmlns="http://www.w3.org/2000/svg" width="24"
                    height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M20 6L9 17L4 12" stroke="#FFFFFF" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
            </div>
        </div>
        <div class="card-footer" :class="{ 'md-card-footer': card.size == 'md', 'sm-card-footer': card.size == 'sm' }">
            <div v-if="card.size == 'sm'" class="sm-header-title">
                <div class="card-title" :style="{ color: isCardSelected(index) ? '#253BFF' : '#98A2B3' }">{{ card.title
                    }}</div>
            </div>
            <div v-if="card.size == 'md'" class="header-title">
                <div class="card-title">{{ card.title }}</div>
...
export default {
    ...
    methods: {
        // Method to toggle the card selection
        toggleCardType(index, event) {
            ...
            if (this.isSingleSelect) {
                ...
                if (this.selectedCards[cardIndex] == index) {
                    this.selectedCards = []
                } else {
